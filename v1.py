...
{M}[*] Email       : XNXX@gmail.com
{H}[*] Bergabung   : 30 JANUARI 1945
{M}[*] ---------------------------------------------
{H}[*] Kadaluwarsa : Sampai Kiamat 
{O}[*] Status      : Premium Sampai Yatim
{H}[*] Author      : JORDY CHAN VICTORIA
{M}[*] ---------------------------------------------
		...
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r {H}[>]{C} {H}STATUS : {H}JORKEP LIVE
  {H}[>]{C} {H}NAMA: {H}{nama}{C}
  ...
  {M}[>]{C} Postingan: {M}{postingan}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			print(f'\n [{U}+{C}] Total Result JORKEP {H}{len(g)}{C}')
